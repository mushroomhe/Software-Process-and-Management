...
<%@ taglib uri="http://localhost:8080/SystemFinal/util" prefix="util"%>
<html>
<head>
			...
			<table class="table table-striped">
				...
				<c:forEach items="${demandBeans }" var="item" varStatus="status">
					<tr>
						...
						<a
							href="${pageContext.request.contextPath }/DemandServlet?method=details&WJID=${item.WJID }">${item.JSXQMC }</a>
							</c:if>
							<c:if test="${item.SFSH==1 }">${item.JSXQMC }</c:if>
							</td>
						<td>${item.createDate }</td>
						<td><c:if test="${item.SFSH==0 }">还未审核</c:if> 	<!-- 未通过可以进行修改 --> 
						<c:if test="${item.SFSH==1 }">通过审核</c:if>
						<c:if test="${item.SFSH==2 }">已退回</c:if></td>
